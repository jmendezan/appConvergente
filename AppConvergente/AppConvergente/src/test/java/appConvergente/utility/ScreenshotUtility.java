package appConvergente.utility;
...
public class ScreenshotUtility {
    ...
    public void takeScreenshotWindows(String UIName) throws IOException, AWTException {
        int val = 1;
        if (contador > 1) {
            val = contador;
        }
    ...
    public void takeScreenshotDriver(String UIName, WebDriver driver) throws IOException {
        int val = 1;
        if (contador > 1) {
            val = contador;
        }
        ...
        if (val > 0 && val < 10) {
            fmt.format("%02d", val);
            FileUtils.copyFile(SrcFile, new File("./results/screenshot/" + fmt + ". " + UIName + " " + timestamp() + ".png"));
            contador = val + 1;
        } else {
            FileUtils.copyFile(SrcFile, new File("./results/screenshot/" + val + ". " + UIName + " " + timestamp() + ".png"));
            contador = val + 1;
        }
    }
